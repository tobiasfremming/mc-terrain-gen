...
float EvaluateFrostHeight(float wx, float wz, FrostParams p, float fw)
{
    uint s = (uint)(int)p.seed;
    float h = p.baseHeight;

    // broad rolling base (interdune-plain equivalent)
    h += MC_Fbm(wx / p.plainScale, wz / p.plainScale, 3, s + 11u, fw / p.plainScale) * p.plainAmplitude;

    // frozen peaks / hills -- reuses the same ridged-fbm machinery as Canyon's
    // mountains, just at Frost's own scale/amplitude
    h += MC_RidgedFbm(wx / p.peakScale, wz / p.peakScale, (int)p.peakOctaves, s + 23u, 0.5, 2.0, fw / p.peakScale) * p.peakAmp;
    ...
    float sastrugiFade = p.sastrugiEnabled > 0.5 ? MC_DetailFade(fw, p.sastrugiWavelengthAcross) : 0.0;
    if (sastrugiFade > 0.0)
    {
        ...
        float ridge = MC_RidgedFbm(v / p.sastrugiWavelengthAcross, u / p.sastrugiWavelengthAlong, soct, s + 41u, 0.5, 2.0, fw / p.sastrugiWavelengthAcross);
        ...
        h += ridge * p.sastrugiAmp * sastrugiFade;
    }
    ...
    float polyFade = p.permafrostEnabled > 0.5
        ? MC_DetailFade(fw, p.polygonEdgeWidth * p.polygonScale) : 0.0;
    if (polyFade > 0.0)
    {
        float f1, f2;
        MC_Worley2(wx / p.polygonScale, wz / p.polygonScale, s + 61u, f1, f2);
        float trough = 1.0 - MC_Smoothstep(0.0, p.polygonEdgeWidth, f2 - f1);
        h -= trough * p.polygonTroughDepth * polyFade;
    }
    ...
    float crevasseFade = p.crevassesEnabled > 0.5
        ? MC_DetailFade(fw, p.crevasseEdgeWidth * p.crevasseScale) : 0.0;
    if (crevasseFade > 0.0)
    {
        float patch = MC_Fbm(wx / p.crevassePatchScale + 5.2, wz / p.crevassePatchScale - 2.9, 2, s + 71u, fw / p.crevassePatchScale);
        ...
        if (patch > 0.001)
        {
            ...
            h -= crack * p.crevasseDepth * patch * crevasseFade;
        }
    }

    // frost heave hummocks: small bumpy ground texture
    if (p.hummocksEnabled > 0.5)
        h += MC_Fbm(wx / p.hummockScale, wz / p.hummockScale, 2, s + 97u, fw / p.hummockScale) * p.hummockAmp;
